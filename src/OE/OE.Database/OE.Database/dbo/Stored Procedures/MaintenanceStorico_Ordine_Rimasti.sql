...
CREATE PROCEDURE [dbo].[MaintenanceStorico_Ordine_Rimasti]
 @BATCH_SIZE INT=1000
,@DEBUG BIT=0
,@LOG BIT=0
AS
BEGIN
--MODIFICHE:
-- 2015-01-13 Sandro: Archivi i rimasti in qualsiasi stato
-- 2020-04-21 Sandro: Ennuple di giorni di archiviazione
	...
	SELECT @NR_GG_MIN = [GiorniOrdiniAltro] FROM [dbo].[GetArchiviazioneMinima]()
	...
	BEGIN TRY
		...
		-- Riferimento agli ordini rimasti nel DB
		SELECT TOP (@TOP) o.ID, o.DataModifica
			...
			WHERE o.DataModifica <= DATEADD(dd, -@NR_GG_MIN, GETDATE())
					AND (DataPrenotazione > DATEADD(YEAR, 5,GETDATE()) --palesemente errata o vuota
						OR o.DataPrenotazione IS NULL)

					-- Filtro con i giorni delle ENNUPLE
					--
					AND	o.DataModifica <= DATEADD(dd, -e.[GiorniOrdiniAltro], GETDATE())
		...
		PRINT 'Inizio loop su Ordini RIMASTI  da storicizzare ' + CONVERT(varchar(40), GETDATE(), 120)
		PRINT ''
