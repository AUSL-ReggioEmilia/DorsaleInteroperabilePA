...
Public Class EventiReinvioByEsito
    ...
    Private Const MAX_PROCESSI_PER_CICLO As Integer = 50
    ...
    Private Sub odsMain_Selected(sender As Object, e As System.Web.UI.WebControls.ObjectDataSourceStatusEventArgs) Handles odsMain.Selected
        Try
            '
            'GESTIONE ERRORE
            '
            If Not GestioneErrori.ObjectDataSource_TrapError(e, LabelError) Then
                ...
                '
                ' SALVO IN SESSIONE LA LISTA DEI REFERTI DA PROCESSARE.
                '
    ...
    Dim mListaOggettiProcessati As New List(Of OggettoProcessato)
    Private Sub ProcessaOggetto()
        Try
            ...
            If Not Me.Sole_ListaOggettiProcessati Is Nothing AndAlso Me.Sole_ListaOggettiProcessati.Count <> 0 Then
                mListaOggettiProcessati = Me.Sole_ListaOggettiProcessati
            End If
            ...
            If Not listaOggettiDaProcessare Is Nothing Then
                Dim dtOggetti As List(Of System.Data.DataRow) = listaOggettiDaProcessare.AsEnumerable().Take(MAX_PROCESSI_PER_CICLO).ToList
                '
                ...
                If dtOggetti IsNot Nothing AndAlso dtOggetti.Count > 0 Then
                    ...
                    For index = 0 To dtOggetti.Count - 1
                        ...
                        '
                        'Metto un Synclock quando prendo la riga e la rimuovo
                        '
                        SyncLock GlobalObject.gObjNotificaRefertiSynclock
                            '
                            'Ottengo la row del data table.
                            '
                            oRow = CType(dtOggetti.Item(index), DataRow)
                            '
                            ' Leggo i dati dell'oggetto prima di rimuoverlo dal dataset
                            '
                            sId = oRow.Item("IdEvento").ToString
                            sAziendaErogante = oRow.Item("AziendaErogante").ToString
                            sNumeroNosologico = oRow.Item("NumeroNosologico").ToString
                            '
                            'Rimuovo la riga dal dataset. In questo modo evito di processre lo stesso referto più volte.
                            '
                            listaOggettiDaProcessare.Rows.Remove(oRow)
                        End SyncLock
                        '
                        'Aggiungo all'oggetto l'id del ricovero.
                        '
                        oOggettoProcessato.Id = sId
                        oOggettoProcessato.AziendaErogante = sAziendaErogante
                        oOggettoProcessato.NumeroNosologico = sNumeroNosologico
                        Try
                            Using ta = New SoleTableAdapters.EventiReinviaPerIdEventoTableAdapter
                                '
                                ' Eseguo la SP SOLE per il processamento del singolo referto
                                '
                                ta.GetDataByIdEvento(New Guid(sId), "DWH-ADMIN: Riprocessamento evento")
                                '
                                ' Se è andata a buon fine allora stato Notifica = "OK" altrimenti scrivo "Errore"
                                '
                                oOggettoProcessato.StatoProcesso = "OK"
                            End Using
                    ...
                    '
                    Me.Sole_ListaOggettiProcessati = mListaOggettiProcessati
                End If
            End If
        ...
        Try
            ...
            If Not Me.Sole_ListaOggettiDaProcessare Is Nothing AndAlso Me.Sole_ListaOggettiDaProcessare.Rows.Count > 0 Then
                ...
                If Me.Sole_ListaOggettiProcessati IsNot Nothing Then
                    lblStatoNotifica.Text = String.Format("Processati {0} di {1} eventi.", Me.Sole_ListaOggettiProcessati.Count.ToString, Me.NumOggettiTrovati.ToString)
                End If
                Timer1.Enabled = True
            Else
                '
    ...
    Private Sub btnEsportaInExcel_Click(sender As Object, e As EventArgs) Handles btnEsportaInExcel.Click
        Try
            ...
            If Not ListOfEntity Is Nothing AndAlso ListOfEntity.Count > 0 Then
                Using str As MemoryStream = Excel.AnyListToExcel(ListOfEntity, "LISTA_EVENTI_PROCESSATI")
                    If str IsNot Nothing Then
                        Response.Clear()
                        Response.Buffer = True
                        Response.AddHeader("content-disposition", "attachment;filename=" & "LISTA_EVENTI_PROCESSATI" & ".xlsx")
    ...
    Private Sub gvMain_Sorting(sender As Object, e As GridViewSortEventArgs) Handles gvMain.Sorting
...
#Region "Oggetti sessione"
    Private Const KEY_LISTA_OGGETTI_SOLE_DA_PROCESSARE As String = "Eventi_BF3D1CD1-211F-4F3E-B5EE-F81B3DBAF367"
    Private Const KEY_LISTA_OGGETTI_SOLE_PROCESSATI As String = "Eventi_B0F88247-F885-4190-B2DD-999D49591C21"
