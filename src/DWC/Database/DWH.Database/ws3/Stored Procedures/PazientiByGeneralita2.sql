...
-- Description:	Rispetto alla versione Ws2 ho aggiunto l'AnnoNascita
--				ATTENZIONE: per ora la ricerca SOUNDEX non la facciamo
...
-- =============================================
CREATE PROCEDURE  [ws3].[PazientiByGeneralita2]
(
  @MaxNumRow	INTEGER
...
BEGIN
	...
	SET @DataNascitaMin = CAST(CAST(@AnnoNascita AS VARCHAR(4)) + '-01-01' AS DATETIME)
	SET @DataNascitaMax = CAST(CAST(@AnnoNascita AS VARCHAR(4)) + '-12-31' AS DATETIME)
	...
	SELECT TOP (@MaxNumRow) 
		...
		--
		-- Informazioni sull'ultimo ricovero: le ricavo facendo con PazientiAnteprima e WS3.Ricoveri
		--
	...
	WHERE	
		(P.Cognome LIKE @Cognome + '%' OR NULLIF(@Cognome, '') IS NULL)
		AND (P.Nome LIKE @Nome + '%'  OR NULLIF(@Nome, '') IS NULL)
		AND (P.LuogoNascita LIKE @LuogoNascita + '%' OR NULLIF(@LuogoNascita, '') IS NULL)
		AND (P.DataNascita = @DataNascita OR @DataNascita IS NULL)
		AND (P.DataNascita BETWEEN @DataNascitaMin and @DataNascitaMax OR @AnnoNascita IS NULL)	
	ORDER BY  
	--Default
