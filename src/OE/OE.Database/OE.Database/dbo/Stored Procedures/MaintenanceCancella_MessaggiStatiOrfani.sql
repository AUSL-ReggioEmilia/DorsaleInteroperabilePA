...
-- =============================================
CREATE PROCEDURE [dbo].[MaintenanceCancella_MessaggiStatiOrfani]
...
-- 2020-04-22 Sandro: Review 
	...
	BEGIN TRY
		...
		DELETE TOP (@TOP) [dbo].[MessaggiStati]
		WHERE DataInserimento <= DATEADD(dd,-@NR_GG_NO_HISTORY,GETDATE())
			AND [IDOrdineErogatoTestata] IS NULL
	...
	PRINT '[' + cast(getdate() as varchar(30)) + '] Cancellazione completata. Totale MessaggiStati eliminati ' 
