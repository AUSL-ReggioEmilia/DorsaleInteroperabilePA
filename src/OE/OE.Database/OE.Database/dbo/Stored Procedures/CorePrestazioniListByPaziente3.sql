...
-- =============================================
CREATE PROCEDURE [dbo].[CorePrestazioniListByPaziente3]
( @utente VARCHAR (64)
...
, @idStato TINYINT=1
, @idSacPaziente UNIQUEIDENTIFIER
, @valore VARCHAR (256)
)
AS
BEGIN
	...
	DECLARE @StoricoGiorni INT = 720 --30
	IF @valore IS NULL OR LEN(@valore) < 2
		SET @StoricoGiorni = 720 --7
	...
	FROM (
		...
		FROM dbo.CorePrestazioniList P
			...
			-- Prestazioni già richieste sul paziente
		...
		WHERE OT.PazienteIdSac = @idSacPaziente
	...
	ORDER BY
		...
		(SELECT MAX(ORR.DataModifica) FROM OrdiniRigheRichieste ORR
				INNER JOIN OrdiniTestate OT ON OT.ID = ORR.IDOrdineTestata
			WHERE ORR.IDPrestazione = P_RIC.ID
				AND OT.PazienteIdSac = @idSacPaziente
				AND OT.DataModifica > DATEADD(DAY, @StoricoGiorni * -1, GETDATE())
			) DESC
		, P_RIC.Codice
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[CorePrestazioniListByPaziente3] TO [DataAccessMsg]
    AS [dbo];


GO
GRANT EXECUTE
    ON OBJECT::[dbo].[CorePrestazioniListByPaziente3] TO [DataAccessWs]
    AS [dbo];
