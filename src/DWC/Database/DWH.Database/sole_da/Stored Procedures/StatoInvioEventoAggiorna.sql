...
-- =============================================
CREATE PROCEDURE [sole_da].[StatoInvioEventoAggiorna]
(
...
BEGIN
	...
	SELECT @DataPartizione = DataPartizione FROM [store].[EventiBase]
		WHERE Id = @IdEvento
	...
	BEGIN
		...
		BEGIN TRY
			...
			EXEC @ret = [dbo].[EventoAttributoAggiorna] @IdEvento, @DataPartizione, @AttribNomeData, @DataInvio

			IF @ret <> 0 RAISERROR ('Error in [dbo].[EventoAttributoAggiorna]! Nome=%s', 16, 1, @AttribNomeData)
			--
			-- Esito (ultimo)
			--
			DECLARE @AttribNomeEsito VARCHAR(64) = '$@Sole-EsitoInvio'
			EXEC @ret = [dbo].[EventoAttributoAggiorna] @IdEvento, @DataPartizione, @AttribNomeEsito, @EsitoInvio

			IF @ret <> 0 RAISERROR ('Error in [dbo].[EventoAttributoAggiorna]! Nome=%s', 16, 1, @AttribNomeEsito)
			--
			-- IdDocumento (ultimo)
			--
			DECLARE @AttribNomeIdDocumento VARCHAR(64) = '$@Sole-IdDocumento'
			EXEC @ret = [dbo].[EventoAttributoAggiorna] @IdEvento, @DataPartizione, @AttribNomeIdDocumento, @IdDocumento

			IF @ret <> 0 RAISERROR ('Error in [dbo].[EventoAttributoAggiorna]! Nome=%s', 16, 1, @AttribNomeIdDocumento)
			--
		...
		BEGIN CATCH
			...
			--
			ROLLBACK
			PRINT 'Errore durante la SP [sole_da].[StatoInvioEventoAggiorna]'
			RETURN @@error
		END CATCH
	END
END