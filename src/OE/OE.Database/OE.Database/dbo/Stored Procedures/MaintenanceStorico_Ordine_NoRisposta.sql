...
CREATE PROCEDURE [dbo].[MaintenanceStorico_Ordine_NoRisposta]
 @BATCH_SIZE INT=1000
,@DEBUG BIT=0
,@LOG BIT=0
AS
...
-- 2020-04-21 Sandro: Ennuple di giorni di archiviazione
	...
	SELECT @NR_GG_MIN = [GiorniOrdiniNoRisposta] FROM [dbo].[GetArchiviazioneMinima]()
	...
	BEGIN TRY
		PRINT 'Inizio query selezione Ordini SENZA RISPOSTA da storicizzare ' + CONVERT(varchar(40), GETDATE(), 120)
		...
		SELECT TOP (@TOP) o.ID, o.DataModifica
			...
			WHERE o.DataModifica <= DATEADD(dd,-@NR_GG_MIN, GETDATE())
					...
					--
					AND o.DataModifica <= DATEADD(dd,-e.[GiorniOrdiniNoRisposta], GETDATE())
					AND (o.DataPrenotazione <= DATEADD(dd,-e.[GiorniOrdiniNoRisposta], GETDATE())
						OR o.DataPrenotazione IS NULL)
					...
					--
					AND dbo.IsOrdineSenzaRispostaOppureStato(e.IdOrdineTestata) = 1
		...
		PRINT 'Inizio loop sui Ordini SENZA RISPOSTA da storicizzare ' + CONVERT(varchar(40), GETDATE(), 120)
		PRINT ''
