...
-- Description:	SP di inserimento paziente usata dai WS (aggiorna un set limitato di campi e viene passata la data sequenza)
-- =============================================
CREATE PROCEDURE [dbo].[PazientiWsBaseInsert2]
	  @Utente AS varchar(64)
	, @DataSequenza datetime
...
DECLARE @Msg AS varchar(500)
	...
	BEGIN
		RAISERROR('Errore di Provenienza non trovata durante PazientiWsBaseInsert2!', 16, 1)
	...
	BEGIN
		RAISERROR('Errore durante PazientiWsBaseInsert2, il campo CodiceFiscale è vuoto!', 16, 1)
		SELECT 2001 AS ERROR_CODE
		GOTO ERROR_EXIT
	END
	...
	IF LEN(@Msg) > 35
		BEGIN
			...
			EXEC dbo.PazientiEventiAvvertimento @Utente, 0, 'PazientiWsBaseInsert2', @Msg
		END
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[PazientiWsBaseInsert2] TO [DataAccessWs]
    AS [dbo];
