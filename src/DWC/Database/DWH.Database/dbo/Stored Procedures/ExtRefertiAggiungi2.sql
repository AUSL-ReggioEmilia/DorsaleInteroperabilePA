...
*/
CREATE PROCEDURE [dbo].[ExtRefertiAggiungi2] 
...
DECLARE @DataModifica  Datetime
	...
	-- Legge la PK del referto (non ho bisogno della @DataPartizione perchè sono in INSERIMENTO, la @DataPartizione viene derivata dalla @DataReferto)
	...
	BEGIN
		...
		------------------------------------------------------
		SELECT INSERTED_COUNT = NULL
		RAISERROR('Errore il referto esiste già!', 16, 1)
		RETURN 1001
	END

	------------------------------------------------------
	--  Cerca IdPaziente e nuovo IdReferto
	------------------------------------------------------	
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[ExtRefertiAggiungi2] TO [ExecuteExt]
    AS [dbo];
