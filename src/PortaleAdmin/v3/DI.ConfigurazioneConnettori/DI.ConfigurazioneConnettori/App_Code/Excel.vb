...
Public Class Excel
    ...
    Public Shared Function AnyListToExcel(Input As IList, WorksheetName As String) As MemoryStream
        ...
        For Each riga In Input
            ...
            For Each colonna As PropertyInfo In properties
                ...
                Dim value = colonna.GetValue(riga, Nothing)
...
#Region "Utilità Linq"
    ...
    Public Shared Sub DataRowToEntity(drOrigin As DataRow, oDestination As Object)
        ...
        For Each column In destColumns
            Dim namedArgumentsList = column.CustomAttributes(0).NamedArguments
            Dim canBeNull = (From item In namedArgumentsList Where item.MemberName = "CanBeNull" Select item.TypedValue.Value).FirstOrDefault
            ...
            If origValue Is DBNull.Value Then
                'SE LA COLONNA E' NULLABILE ALLORA RESTITUISCO NULL
                If CType(canBeNull, Boolean) Then
                    destValue = Nothing
                Else
                    'SE SONO QUI ALLORA LA COLONNA NON E' NULLABILE.
                    'OOTENGO IL VALORE DI DEFAULT IN BASE AL TYPE DELLA COLONNA CORRENTE.
                    destValue = GetDefaultValueType(destType)
                End If
            Else
    ...
    ''' <summary>
    ''' RESTITUISCE IL DEFAULT VALUE DEL TYPE DELLA COLONNA DI DESTINAZIONE.
    ''' </summary>
    ''' <param name="destType"></param>
    ''' <returns></returns>
    Private Shared Function GetDefaultValueType(destType As Type) As Object
