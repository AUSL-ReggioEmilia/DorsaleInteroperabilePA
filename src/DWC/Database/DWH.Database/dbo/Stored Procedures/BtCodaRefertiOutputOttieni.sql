
CREATE PROCEDURE [dbo].[BtCodaRefertiOutputOttieni]
AS
BEGIN
/*
	Modifica Ettore 2012-12-17: 
		per evitare deadlock utilizzando la proprietà PoolWhileDataFound=true della porta WCF
		di BizTalk è stata impostata una transazione all'interno della SP per utilizzare un
		livello di serializzazione meno forte di quello usato da BizTalk e degli HINT sulla 
		SELECT che ottiene l'id del record da inviare a BizTalk:
			l'hint READPASTE viene usato "saltare" record già bloccati da un'altra transazione
			l'hint UPDLOCK viene usato per bloccare il record per tutta la durata della transazione 
			
	Modifica Sandro 2015-06-20:		Compressione campo messaggio
	Modifica Sandro 2015-11-20:		Dati minimi XML non compressi
	Modify date: 2017-02-03 ETTORE: Gestione con Semaforo
	Modify date: 2019-02-28 Sandro: Se priorità > 7 solo di notte
	Modify date: 2020-10-02 Sandro: Se il campo messaggio è NULL calcolo XMl con [dbo].[GetRefertoXml2](@IdReferto)
										la SP [dbo].[ExtRefertiAfterProcess] per velocizzare le operazioni non lo farà
	Modify date: 2020-10-06 SAndro: Esclude i referti non più nel dwh, rimarranno nella coda, da rivedere
*/

	SET NOCOUNT ON;

	--/////////////////////////////////////////////////////////////////////////////////////////////////////
	--   Nuovo gestione di accesso sequenziale tramite semaforo per uso con più host di BT
	--/////////////////////////////////////////////////////////////////////////////////////////////////////

	DECLARE @TimeOutReadSecond INT = 60
	--
	-- Controllo [HostBiztalk] con priorità di risposta
	--
	DECLARE @HostName VARCHAR(64) = CONVERT(VARCHAR(64), @@SPID)
	SELECT @HostName = RTRIM(hostname) FROM sys.sysprocesses WHERE spid = @@SPID

	DECLARE @HostNamePriority VARCHAR(64) = ''
	SELECT TOP 1 @HostNamePriority = [HostBiztalk]
	FROM [dbo].[CodaRefertiOutputSemaforo] WITH(TABLOCKX)
	WHERE [DataLettura] > DATEADD( SECOND, @TimeOutReadSecond * -1, GETUTCDATE())
	ORDER BY [Primario] DESC, [HostBiztalk]

	--/////////////////////////////////////////////////////////////////////////////////////////////////////
	--
	-- Esegue solo sul PRIMARIO
	--

	IF @HostNamePriority = @HostName
	BEGIN

		DECLARE @Now DATETIME
		DECLARE @IdSequenza INT

		DECLARE @IdReferto UNIQUEIDENTIFIER
		DECLARE @Messaggio XML		
		--
		-- Imposto il livello di serializzazione che verrà usato dalla SP (meno forte di quello di default di BizTalk)
		--
		SET TRANSACTION ISOLATION LEVEL READ COMMITTED

		BEGIN TRANSACTION 
		BEGIN TRY
			--
			-- FONDAMENTALE: si deve restituire un solo record per volta
			-- 2019-02-28 -se priorità > 7 solo di notte
			--
			-- 2020-10-06 esclude i referti non più nel dwh
			--
			SELECT TOP 1 @IdSequenza=IdSequenza
			FROM [CodaRefertiOutput] WITH (READPAST, UPDLOCK)
			WHERE ([OrdineInvio] <= 7	
					OR	(DATEPART(HOUR, GETDATE()) > 19 OR DATEPART(HOUR, GETDATE()) < 7)
					)
				AND (EXISTS (SELECT * FROM [store].[RefertiBase]
							 WHERE [RefertiBase].Id = [CodaRefertiOutput].IdReferto
							)
					OR [CodaRefertiOutput].Operazione = 2
					)

			ORDER BY OrdineInvio ASC, IdSequenza ASC
		
			IF NOT @IdSequenza IS NULL
			BEGIN 
				SET @Now = GETUTCDATE()
				--
				-- Controllo se presente XML referto
				--
				SELECT @Messaggio = [Messaggio]
					 , @IdReferto = [IdReferto]
				FROM [CodaRefertiOutput] AS [RefertiOutput]
				WHERE [IdSequenza] = @IdSequenza

				IF @Messaggio IS NULL AND NOT @IdReferto IS NULL
					SET @Messaggio = [dbo].[GetRefertoXml2](@IdReferto)
				--
				-- Inserisco record nello storico			
				--
				INSERT INTO [CodaRefertiOutputInviati]
					([DataInvio], [IdSequenza], [DataInserimento], [IdReferto], [Operazione]
						, [IdCorrelazione], [CorrelazioneTimeout], [OrdineInvio]
						, [Messaggio], [MessaggioCompresso])
				SELECT @Now ,[IdSequenza], [DataInserimento], [IdReferto], [Operazione]
						, [IdCorrelazione], [CorrelazioneTimeout], [OrdineInvio]
						, dbo.TrasformRefertoXmlForCodaInviati(@Messaggio)  
						, dbo.compress(CONVERT(VARBINARY(MAX), @Messaggio))
				FROM [CodaRefertiOutput]
				WHERE [IdSequenza] = @IdSequenza
				--
				-- Restituisco al chiamante (BizTalk) l'XML
				-- CorrelazioneTimeout in minuti
				--
				SELECT [IdSequenza]
					, @Now AS [DataInvio]
					, [DataInserimento] AS [DataLog]
					, [Operazione]				
					, [IdCorrelazione] 
					, [CorrelazioneTimeout]
					, @Messaggio AS [Messaggio] 
				FROM [CodaRefertiOutput] AS [RefertiOutput]
				WHERE [IdSequenza] = @IdSequenza
				--
				-- Cancello  la restituita
				--
				DELETE FROM [CodaRefertiOutput] WHERE [IdSequenza] = @IdSequenza
			END
			--
			-- Commit delle modifiche
			--
			COMMIT
		END TRY
		BEGIN CATCH
			--
			-- Rollback delle modifiche
			--
			IF @@TRANCOUNT > 0
				ROLLBACK
			--
			-- Raise dell'errore
			--
			DECLARE @xact_state INT
			DECLARE @msg NVARCHAR(2000)
			SELECT @xact_state = xact_state(), @msg = error_message()

			DECLARE @report NVARCHAR(4000);
			SELECT @report = N'BtCodaRefertiOutputOttieni. In catch: ' + @msg + N' xact_state:' + cast(@xact_state AS NVARCHAR(5));
			RAISERROR(@report, 16, 1)
		END CATCH

		--
		--  Ritorna 0 se non c'è XML
		--
		RETURN CASE WHEN @Messaggio IS NULL THEN 0 ELSE 1 END

	END ELSE BEGIN
		RETURN 0
	END 

END


GO
GRANT EXECUTE
    ON OBJECT::[dbo].[BtCodaRefertiOutputOttieni] TO [ExecuteBiztalk]
    AS [dbo];

