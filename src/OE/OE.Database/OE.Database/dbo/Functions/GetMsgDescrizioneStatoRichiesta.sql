...
-- Description:	Stato dell'intera richiesta, se MULTI SETTORE valuta tutte le testate
--				Copiata da [GetWsDescrizioneStato2]
--				Usata da tutte le SP MsgOrdiniErogatiTestateSelect-xxx
-- =============================================
CREATE FUNCTION [dbo].[GetMsgDescrizioneStatoRichiesta]
(
...
--Annullato = 10
	...
	IF @stato = 'CA'
		INSERT INTO @RetStato (Codice, Descrizione)
					VALUES (@stato, 'Cancellato')

	ELSE IF @stato = 'HD'
		INSERT INTO @RetStato (Codice, Descrizione)
					VALUES (@stato, 'Inserito')
	...
	BEGIN
		...
		--Calcolo GLOBALE stato tutte le righe Erogante
		SET @statoErogato = dbo.GetMinStatoErogatoTestateOrderEntry(@idOrdineTestata)
		...
		-- Riga di output
		INSERT INTO @RetStato (Codice, Descrizione)
					VALUES (@statoErogato, @descrErogato)
	END ELSE BEGIN
		--
