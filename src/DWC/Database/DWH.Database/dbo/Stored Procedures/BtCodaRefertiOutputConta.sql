...
-- =============================================
CREATE PROCEDURE [dbo].[BtCodaRefertiOutputConta]
AS
BEGIN
	SET NOCOUNT ON
	...
	BEGIN TRY
		...
		IF EXISTS (SELECT * FROM [dbo].[CodaRefertiOutputSemaforo] WITH(TABLOCKX) WHERE [HostBiztalk] = @HostName)
			UPDATE [dbo].[CodaRefertiOutputSemaforo] WITH(TABLOCKX)
		...
		ELSE
			INSERT INTO [dbo].[CodaRefertiOutputSemaforo] WITH(TABLOCKX) ([HostBiztalk], [DataLettura], [Primario])
		...
		BEGIN
			SELECT COUNT(*) FROM CodaRefertiOutput WITH (NOLOCK)
	...
	BEGIN CATCH
		...
		SELECT @report = N'BtCodaRefertiOutputConta. In catch: ' + @msg + N' xact_state:' + cast(@xact_state AS NVARCHAR(5));
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[BtCodaRefertiOutputConta] TO [ExecuteBiztalk]
    AS [dbo];
