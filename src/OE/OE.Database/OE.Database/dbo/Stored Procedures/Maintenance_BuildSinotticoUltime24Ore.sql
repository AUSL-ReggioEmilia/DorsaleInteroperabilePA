...
-- Description: Riempie la tabella SinotticoUltime24Ore con i dati aggregati per sistema 
--				dalla @dataDa = GETDATE()-24 ore ore a @dataA = GETDATE()
-- =============================================
CREATE PROCEDURE [dbo].[Maintenance_BuildSinotticoUltime24Ore]
AS
BEGIN
	SET NOCOUNT ON
	DECLARE @RowCount INTEGER = 0

	DECLARE @dataDa DATETIME = DATEADD(hour, -24, GETDATE()) --indietro di 24 ore
	DECLARE @dataA DATETIME = GETDATE() --fino alla data-ora corrente
	...
	--
	--DELETE SinotticoUltime24Ore 
	--WHERE Giorno BETWEEN @dataDa AND @dataA	
	--SELECT @RowCount = @@ROWCOUNT
	--PRINT 'RIGHECANCELLATE: ' + CAST(@RowCount AS VARCHAR(10))

	TRUNCATE TABLE SinotticoUltime24Ore 
		
	INSERT INTO SinotticoUltime24Ore
	(	 [Id]
		,[IdSistemaErogante]
		,[IdSistemaRichiedente]
		,[Stato]
