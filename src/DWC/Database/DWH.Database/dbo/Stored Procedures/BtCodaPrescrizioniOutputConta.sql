...
-- =============================================
CREATE PROCEDURE [dbo].[BtCodaPrescrizioniOutputConta]
AS
BEGIN
	SET NOCOUNT ON
	...
	BEGIN TRY
		...
		IF EXISTS (SELECT * FROM [dbo].[CodaPrescrizioniOutputSemaforo] WITH(TABLOCKX) WHERE [HostBiztalk] = @HostName)
			UPDATE [dbo].[CodaPrescrizioniOutputSemaforo] WITH(TABLOCKX)
		...
		ELSE
			INSERT INTO [dbo].[CodaPrescrizioniOutputSemaforo] WITH(TABLOCKX) ([HostBiztalk], [DataLettura], [Primario])
		...
		BEGIN
			...
			SELECT COUNT(*) 
			FROM dbo.CodaPrescrizioniOutput  WITH(NOLOCK)
	...
	BEGIN CATCH
		...
		SELECT @report = N'BtCodaPrescrizioniOutputConta. In catch: ' + @msg + N' xact_state:' + cast(@xact_state AS NVARCHAR(5));
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[BtCodaPrescrizioniOutputConta] TO [ExecuteBiztalk]
    AS [dbo];
