...
Public Class UtentiAggiungiLista
		...
		Try
			For Each row As GridViewRow In gvLista.Rows
				If DirectCast(row.FindControl("CheckBox"), CheckBox).Checked Then
					iNumChecked += 1
					Dim sUtente As String = gvLista.DataKeys(row.RowIndex).Values("Utente")
					Dim sDesc As String = Utility.StringEmptyDBNullToNothing(gvLista.DataKeys(row.RowIndex).Values("Descrizione"))
					Dim sEmail As String = Utility.StringEmptyDBNullToNothing(gvLista.DataKeys(row.RowIndex).Values("Email"))
					If Not Add(sUtente, sDesc, Nothing, sEmail, 0) Then
						iErrori += 1
					End If
				End If
			Next
