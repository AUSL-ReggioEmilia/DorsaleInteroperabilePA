...
-- Description:	Restituisce i referti associati ad un nosologico e i referti associati alla lista di attesa che ha generato il nosologico
-- Modify date: 2017-10-09 - ETTORE: Gestione filtro basato sul consenso 
-- Modify date: 2019-01-08 - ETTORE: Eliminato l'uso delle costanti ASMN e AUSL usate per invocare la funzione dbo.GetCodicePrenotazioneByAziendaEroganteCodiceRicovero()
--									 Corretto errore: quando si cerca i referti bisogna cercare sia per Id Paziente che Id Paziente padre, cioè per l'intera catena di fusione
--									 Ho aggiunto nella colonna Id dell tabella @Pazienti gli Id dei padri della fusione
-- =============================================
CREATE PROCEDURE ws2.RefertiByNosologico
(
...
BEGIN
/*
	Sostituisce la dbo.Ws2RefertiByNosologico
...
*/
	...
	SELECT @DallaDataMin = MIN(DallaData) FROM @TabPazienti_Date
	SELECT @AllaDataMax = MAX(AllaData) FROM @TabPazienti_Date
	...
	--
	-- Determino il paziente attivo a partire da un paziente  della tabella @TabPazienti_Date_Date (devono appartenere alla stessa catena di fusione)
	--
	...
	WHERE	
		(
			--La tabella @TAB_Nosologici DEVE contenere TUTTI I nosologici: quello del ricovero e quello della eventuale lista di attesa			
