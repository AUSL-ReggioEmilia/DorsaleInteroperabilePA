...
-- =============================================
CREATE PROCEDURE  [dbo].[BtPazientiQueueConta]
AS
BEGIN
	SET NOCOUNT ON
	...
	BEGIN TRY
		...
		IF EXISTS (SELECT * FROM [dbo].[PazientiQueueSemaforo] WITH(TABLOCKX) WHERE [HostBiztalk] = @HostName)
			UPDATE [dbo].[PazientiQueueSemaforo] WITH(TABLOCKX)
		...
		ELSE
			INSERT INTO [dbo].[PazientiQueueSemaforo] WITH(TABLOCKX) ([HostBiztalk], [DataLettura], [Primario])
		...
		BEGIN
			select COUNT(*)	from PazientiQueue WITH(nolock)
			RETURN 1
		END ELSE BEGIN
			SELECT 0
			RETURN 0
	...
	BEGIN CATCH
		...
		SELECT @report = N'BtPazientiQueueConta. In catch: ' + @msg + N' xact_state:' + cast(@xact_state AS NVARCHAR(5));
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[BtPazientiQueueConta] TO [ExecuteBiztalk]
    AS [dbo];
