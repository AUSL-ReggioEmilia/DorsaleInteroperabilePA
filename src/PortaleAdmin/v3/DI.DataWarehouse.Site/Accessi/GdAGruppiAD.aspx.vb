...
Public Class GdAGruppiAD
	Inherits System.Web.UI.Page

	Private ReadOnly msPAGEKEY As String = Page.GetType().BaseType.FullName
	Private Const KEY_DESC_GDA As String = "KEY_DESC_GDA"
	...
	Private Sub Page_PreRenderComplete(sender As Object, e As System.EventArgs) Handles Me.PreRenderComplete
		Try
					...
					Using ta As New GdATableAdapter()
						Using dt = ta.GetData(New Guid(msID), Nothing, Nothing)
							If dt.Rows.Count = 1 Then
								Dim dr = dt(0)
								ViewState.Add(KEY_DESC_GDA, dr.AziendaCodice & " - " & dr.Descrizione)
							Else
			...
			If mbModalitàInserimento Then
				lblTitolo.Text = "Selezionare i Gruppi amministrativi di: " & ViewState(KEY_DESC_GDA)
			Else
				lblTitolo.Text = "Gruppi amministrativi di: " & ViewState(KEY_DESC_GDA)
			End If
		...
		Try
			For Each row As GridViewRow In gvLista.Rows
				...
				If chkItem.Checked Then
					...
					odsLista.InsertParameters("NomeGruppoAD").DefaultValue = gvLista.DataKeys(row.RowIndex).Values("Gruppo").ToString
			...
			Else
				Utility.ShowErrorLabel(LabelError, "Selezionare i Gruppi da associare al GdA.")
			End If
