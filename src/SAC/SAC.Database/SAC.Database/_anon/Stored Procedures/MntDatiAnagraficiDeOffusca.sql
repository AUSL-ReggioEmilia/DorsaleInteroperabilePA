...
-- Description:	DeOffusca i dati anagrafici presenti nel SAC (tabella Pazienti)
-- =============================================
CREATE PROCEDURE [_anon].[MntDatiAnagraficiDeOffusca]
(
	@MaxRow INTEGER = 100
	, @Simulazione BIT = 1
...
BEGIN
	SET NOCOUNT ON;
	PRINT '*************************************************'
	PRINT 'DEOFFUSCAMENTO DEI DATI ANAGRAFICI SAC'
	...
	FROM dbo.Pazienti 
	WHERE 
		(LEFT(Cognome,1) = '$' OR LEFT(Nome,1) = '$')  --condizione per DeOffuscare
	...
	BEGIN
		...
		BEGIN
			BEGIN TRY
				...
				-- DeCritto
				--
				EXEC _anon.MntDatiAnagraficiOffuscaDeOffuscaById 'DeOffusca', @Simulazione, @Id, @Cognome, @Nome,  @CognomeNuovo OUTPUT, @NomeNuovo OUTPUT
			...
			BEGIN CATCH
				...
				SELECT @report = CHAR(9) + N'MntDatiAnagraficiDeOffusca. In catch: ' + @msg + N' xact_state:' + cast(@xact_state AS NVARCHAR(5));
				PRINT @report;	
