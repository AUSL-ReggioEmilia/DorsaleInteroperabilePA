...
CREATE PROCEDURE [ws3].[RicoveriByPaziente2]
(
...
*/
	...
	WHERE
		...
		--
		AND (DataAccettazione >= @DallaData OR @DallaData IS NULL) AND (DataAccettazione <= @AllaData OR @AllaData IS NULL)
		AND (DataPartizione >= @DallaDataPartizione OR @DallaDataPartizione IS NULL)
