	...
	Public Shared Sub SetWCFCredentials(oEndPointBinding As ServiceModel.Channels.Binding, oCredentials As ServiceModel.Description.ClientCredentials, sUser As String, sPassword As String)
		...
		If oBasicHttpBinding IsNot Nothing Then
			...
			If oCredType = HttpClientCredentialType.Basic Then
				...
				'DiagnosticsHelper.WriteInformation("SetWCFCredentials HttpClientCredentialType.Basic, Utente=" & sUser)
				Exit Sub

			ElseIf oCredType = HttpClientCredentialType.Windows Then
				' Autenticazione WINDOWS con utente e password
				If Not String.IsNullOrEmpty(sUser) AndAlso Not String.IsNullOrEmpty(sPassword) Then
					...
					'DiagnosticsHelper.WriteInformation("SetWCFCredentials HttpClientCredentialType.Windows, Utente=" & sUser)
				Else
					'Integrated autentication                
					oCredentials.Windows.ClientCredential = CType(CredentialCache.DefaultCredentials(), NetworkCredential)

					'DiagnosticsHelper.WriteInformation("SetWCFCredentials HttpClientCredentialType.Windows, Utente=nullo")
				End If
