...
Namespace DI.OrderEntry.Admin
	...
	Public Class Ennuple
		Inherits Page

		Private Const PageSessionIdPrefix As String = "Ennuple_"
		...
		Public Sub SaveButton_Click()
			...
			Dim idGruppiPrestazioni As Guid? = Nothing
			If Not Me.EnnuplaDetail_GruppiPrestazioniDropDownList.SelectedValue Is Nothing AndAlso Not String.IsNullOrEmpty(Me.EnnuplaDetail_GruppiPrestazioniDropDownList.SelectedValue.ToString) Then
				idGruppiPrestazioni = New Guid(Me.EnnuplaDetail_GruppiPrestazioniDropDownList.SelectedValue.ToString)
			End If
			...
			If inverso And
			 ...
			 And idGruppiPrestazioni.HasValue = False _
			 ...
			 And idPriorita Is Nothing Then
				...
				ErrorLabel.Text = "Non è possibile inserire un'Ennupla che neghi l'accesso a tutte le prestazioni."
				Return
			End If

			Try
				Using uiEnnupleAccessiListTableAdapter As New DI.OrderEntry.Admin.Data.EnnupleTableAdapters.UiEnnupleListTableAdapter()

					If id.HasValue AndAlso id.Value <> Guid.Empty Then
						'modifica
						uiEnnupleAccessiListTableAdapter.Update(id, My.User.Name, idGruppoUtenti, idGruppiPrestazioni, descrizione, note, orarioInizio, orarioFine, lunedi, martedi, mercoledi, giovedi, venerdi, sabato, domenica, idUnitaOperativa, idSistemaRichiedente, idRegime, idPriorita, inverso, idStato)
					Else
						'inserimento
						uiEnnupleAccessiListTableAdapter.Insert(My.User.Name, idGruppoUtenti, idGruppiPrestazioni, descrizione, note, orarioInizio, orarioFine, lunedi, martedi, mercoledi, giovedi, venerdi, sabato, domenica, idUnitaOperativa, idSistemaRichiedente, idRegime, idPriorita, inverso, idStato)
					End If
