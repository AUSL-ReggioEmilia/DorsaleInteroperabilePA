<%@ Control Language="vb" AutoEventWireup="false" CodeBehind="ucDettaglioPaziente2.ascx.vb" Inherits=".ucDettaglioPaziente2" %>
<div id="dettaglio">
<div class="row">
    <div class="col-sm-12">
        ...
        <div class="panel panel-default small">
            <div class="panel-body">
                ...
                <asp:FormView ID="fvPaziente" runat="server" DataSourceID="odsPaziente" RenderOuterTable="False">
                    <ItemTemplate>
                        <div class="col-sm-6">
                            <a id="SacHyperLink" style="font-weight: bold; font-size: 16px;" href='<%# DI.OrderEntry.User.CustomDataSourceDettaglioPaziente.GetSacPazienteUrl(Eval("Id")) %>' target="_blank" title="visualizza il dettaglio del paziente">
                            ...
                            <br />
                            <label style="font-weight: bold; padding-left: 22px;"><%# Eval("CodiceFiscale") %></label>, 
                            <%# If(Eval("Sesso") = "M", "nato il ", "nata il ") %>
                            <label style="font-weight: bold;"><%# String.Format("{0:d} {1}", Eval("DataNascita"), GetAge2(Eval("DataNascita"))) %></label>
                            <label style="font-weight: bold;"><%# ctype(Eval("ComuneNascita"), WcfDwhClinico.CodiceDescrizioneType).Descrizione %></label>
                        </div>

                        <div id="divIcons" class="col-sm-6 text-right">
                            <asp:Label ID="lblNosologico" runat="server" Text='<%# GetNosologico() %>' Visible="true" /><br />
                            <asp:LinkButton runat="server" ID="lnkReferti" Visible="false" alt='visualizza i referti' OnClick="ShowReferti" title='visualizza i referti'>
                                <img id="imgReferti" runat="server" src=".." />
                            </asp:LinkButton>
                            ...
                            <asp:LinkButton runat="server" ID="lnkRicoveri" Visible="false" OnClick="ShowRicovero" ClientIDMode="Static"><%# GetElement(Eval("Episodio"), Eval("ConsensoAziendale")) %></asp:LinkButton>
                        </div>
                    </ItemTemplate>
                </asp:FormView>
            </div>
...
<asp:ObjectDataSource ID="odsPaziente" runat="server" OldValuesParameterFormatString="original_{0}" SelectMethod="GetDataById" TypeName="DI.OrderEntry.User.CustomDataSourceDettaglioPaziente+PazienteDwh">
    <SelectParameters>
				...
				<div class="row">
					<div class="col-sm-12">
						<asp:GridView ID="gvEsenzioni" AutoGenerateColumns="False" CssClass="small table table-bordered table-striped table-condensed" runat="server" DataSourceID="odsEsenzioni">
							<Columns>
								<asp:BoundField DataField="CodiceEsenzione" HeaderText="Codice" SortExpression="CodiceEsenzione"></asp:BoundField>
								<asp:BoundField DataField="TestoEsenzione" HeaderText="Descrizione" SortExpression="TestoEsenzione"></asp:BoundField>
								<asp:BoundField DataField="DataInizioValidita" DataFormatString="{0:dd/MM/yyyy}" HeaderText="Inizio Validità" SortExpression="DataInizioValidita"></asp:BoundField>
								<asp:BoundField DataField="DataFineValidita" DataFormatString="{0:dd/MM/yyyy}" HeaderText="Fine Validità" SortExpression="DataFineValidita"></asp:BoundField>
								<asp:BoundField DataField="CodiceDiagnosi" HeaderText="CodiceDiagnosi" SortExpression="CodiceDiagnosi"></asp:BoundField>
								<asp:BoundField DataField="DecodificaEsenzioneDiagnosi" HeaderText="Diagnosi" SortExpression="DecodificaEsenzioneDiagnosi"></asp:BoundField>
							</Columns>
						</asp:GridView>
						<asp:ObjectDataSource runat="server" ID="odsEsenzioni" OldValuesParameterFormatString="{0}" SelectMethod="GetDataByIdPaziente" TypeName="DI.OrderEntry.User.CustomDataSourceDettaglioPaziente+Esenzioni">
							<SelectParameters>
								<asp:Parameter Name="idPaziente" Type="String"></asp:Parameter>
							</SelectParameters>
						</asp:ObjectDataSource>
