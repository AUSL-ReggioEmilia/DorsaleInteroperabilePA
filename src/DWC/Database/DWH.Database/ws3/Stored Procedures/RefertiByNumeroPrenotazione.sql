...
CREATE PROCEDURE [ws3].[RefertiByNumeroPrenotazione]
(
...
/*
	CREATA DA ETTORE 2016-03-15:
		Restituisce la lista dei referti per NumeroPrenotazione, associati a richieste esterne all'ospedale
		(cioè con NumeroNosologico non valorizzato)
		NOVITA':
		Ho aggiunto il filtro per DataPartizione e il filtro per DataDal/DataAl su DataEvento
		che mancavano nella versione [ws2].[RefertiByNumeroPrenotazione]
...
BEGIN
	...
	--
	-- Cerco nella ws3.Referti il/i pazienti associati al NumeroPrenotazione
	--
	SELECT TOP 1 @IdPaziente = IdPaziente
	FROM ws3.Referti
	WHERE NumeroPrenotazione = @NumeroPrenotazione
		And ISNULL(NumeroNosologico,'') = '' --questa comporta che vengono restituiti solo prenotazioni esterne cioè CUP
	...
	WHERE 
		...
		AND NumeroPrenotazione = @NumeroPrenotazione 
		--
