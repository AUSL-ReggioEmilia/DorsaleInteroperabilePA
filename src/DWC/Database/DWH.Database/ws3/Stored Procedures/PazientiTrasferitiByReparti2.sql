...
CREATE PROCEDURE [ws3].[PazientiTrasferitiByReparti2]
(
	@IdToken			UNIQUEIDENTIFIER --per ora non lo uso
	, @MaxNumRow		INTEGER
	, @Ordinamento		VARCHAR(128)
	, @RepartiRicovero	AS RepartiRicovero READONLY
	, @TipoRicoveroCodice VARCHAR(16)		--Valori=O,P,S,D,B (valori del campo RicoveriBase.StatoCodice)	
...
/*
	...
	Restituisce tutti i pazienti che hanno subito un trasferimento (anche se già dimessi)
	Aggiunto i parametri @DataNascita, @AnnoNascita e @LuogoNascita per renderla più simile alla [ws3].[PazientiRicoveratiByReparti]
	...
	MODIFICA SANDRO 2016-11-30: Ricerca dati paziente in due parti perchè  la ricerca dei consensi è onerosa
	MODIFICA ETTORE 2017-05-16: MOdificato il TOP nella ricerca pazienti da 10000 a 30000
	...
	MODIFICA SANDRO 2017-06-28: Introdotto ModalitaRicerca = [Ricoveri | Paziente | RicoveriPaziente]
											...
											RicoveriPaziente
	MODIFICA ETTORE 2017-07-20: Aggiunto il filtro su oscuramenti: 
								nella ricerca degli eventi (da cui si deriva Azienda e Nosologico) filtro SOLO per le unita operative
								nella ricerca dei ricoveri filtro gli Oscuramenti
	MODIFICA ETTORE 2017-10-12: Aggiunto join con la tabella @TempRicoveri nella parte 'Integra dati paziente'
								Numero di caratteri per eseguire @ModalitaRicerca = 'Paziente': > 7
	MODIFICA 2017-11-21 ETTORE: Aggiunto i campi di anteprima delle note anamnestiche
*/  
	SET NOCOUNT ON
/*
...
---------------------------------------------------------------------------
	...
	(	AziendaErogante VARCHAR(16) NOT NULL,
		...
		UNIQUE CLUSTERED (AziendaErogante,NumeroNosologico) 
	)
	...
	WHERE
		...
		AND Ev.TipoEventoCodice IN ('A','T')
	...
	BEGIN
		...
		SELECT Id, UPPER(Nome), UPPER(Cognome), CodiceFiscale, DataNascita, LuogoNascitaCodice, LuogoNascitaDescrizione, Sesso
			, DomicilioComuneCodice, DomicilioComuneDescrizione, DomicilioCap, DomicilioIndirizzo
			, ConsensoAziendaleCodice, ConsensoAziendaleDescrizione, ConsensoAziendaleData, DataDecesso					
