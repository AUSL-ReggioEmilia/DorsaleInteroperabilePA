Public Class PcRichiedentiStampantiDetail
    ...
    Private Property IdPcRichiedenti() As String
        Get
            Return DirectCast(ViewState("ViewState-IdPcRichiedenti"), String)
        End Get
        Set(ByVal value As String)
            ViewState("ViewState-IdPcRichiedenti") = value
        End Set
    End Property
    ...
    '
    Private Const PAGE_LIST As String = "PcRichiedentiDetail.aspx"

    Private Const PAGE_DETAIL As String = "PcRichiedentiStampantiDetail.aspx"
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
            ...
            If Not Page.IsPostBack Then
                ...
                '
                IdPcRichiedenti = Request.Params("IdPcRichiedenti")
                '
                ' Leggo dettaglio del sistema erogante per creare un titolo
                '
                lblTitle.Text = GetTitle(IdPcRichiedenti, _id)
                '
                ...
                If SiteMap.CurrentNode IsNot Nothing Then
                    ...
                    SiteMap.CurrentNode.ParentNode.Url = String.Concat(SiteMap.CurrentNode.ParentNode.Url.Split("?"c)(0), "?id=", IdPcRichiedenti)
                End If
                '
                ' Set FormView & Title
                '
...
#Region "Redirect"
    ...
    Private Sub ParentRedirect()
        ...
        Response.Redirect(Me.ResolveUrl("~/" & PAGE_LIST) & "?id=" & IdPcRichiedenti, False)
    End Sub
...
#Region "MainFormDataSource"
    ...
    Protected Sub MainDataSource_Selected(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.ObjectDataSourceStatusEventArgs) Handles MainDataSource.Selected
        Try
            ...
            Else
                '
                ' Memorizzo IdPcRichiedenti
                '
                Dim odt As DataAccess.PcRichiedentiDataSet.PcRichiedentiStampantiSelectDataTable = CType(e.ReturnValue, DataAccess.PcRichiedentiDataSet.PcRichiedentiStampantiSelectDataTable)
                If Not odt Is Nothing AndAlso odt.Rows.Count > 0 Then
                    IdPcRichiedenti = odt(0).IdPcRichiedenti.ToString
    ...
    Private Function MyValidate(ByVal oValue As IOrderedDictionary) As String
        ...
        Dim sServerDiStampa As String = String.Empty
        Dim sStampante As String = String.Empty
        Dim sDescrizione As String = String.Empty
        Dim iTipiSTampante As TipiSTampanteEnum
        Dim sTipoModulo As String = String.Empty
        Dim sFormatoModulo As String = String.Empty
        Dim sServerVirtuale As String = String.Empty
        '
        ' Business Logic
        '
        For Each sKey As String In oValue.Keys
            If sKey = "ServerDiStampa" Then
                sServerDiStampa = oValue(sKey).ToString

            ElseIf sKey = "Stampante" Then
                sStampante = oValue(sKey).ToString
                If String.IsNullOrEmpty(sStampante) Then sMsgError = sMsgError & String.Concat(vbCrLf, String.Format("Il campo '{0}' è obbligatorio.", "Stampante", "<br />"))

            ElseIf sKey = "IdTipiStampante" Then
                iTipiSTampante = CType(oValue(sKey), TipiSTampanteEnum)

            ElseIf sKey = "TipoModulo" Then
                sTipoModulo = oValue(sKey).ToString
                If String.IsNullOrEmpty(sTipoModulo) Then sMsgError = sMsgError & String.Concat(vbCrLf, String.Format("Il campo '{0}' è obbligatorio.", "TipoModulo", "<br />"))

            ElseIf sKey = "FormatoModulo" Then
                sFormatoModulo = oValue(sKey).ToString
                If String.IsNullOrEmpty(sFormatoModulo) Then sMsgError = sMsgError & String.Concat(vbCrLf, String.Format("Il campo '{0}' è obbligatorio.", "FormatoModulo", "<br />"))

            ElseIf sKey = "ServerVirtuale" Then
                sServerVirtuale = oValue(sKey).ToString
                If String.IsNullOrEmpty(sServerVirtuale) Then sMsgError = sMsgError & String.Concat(vbCrLf, String.Format("Il campo '{0}' è obbligatorio.", "ServerVirtuale", "<br />"))

            ElseIf sKey = "IdPcRichiedenti" Then
                'Non faccio niente
...
#Region "Funzioni chiamate nel markup"
    ...
    Protected Function GetNewItemNavigateUrl() As String
        Return Me.ResolveUrl("~/PcRichiedentiStampantiDetail.aspx")
    End Function

#End Region

    Private Function GetTitle(ByVal sIdPcRichiedente As String, sId As String) As String
        Dim sRet As String = String.Empty
        Try
            Using oTa As New DataAccess.PcRichiedentiDataSetTableAdapters.PcRichiedentiSelectTableAdapter
                Using oDt As DataAccess.PcRichiedentiDataSet.PcRichiedentiSelectDataTable = oTa.GetData(New Guid(sIdPcRichiedente))
                    If Not oDt Is Nothing AndAlso oDt.Rows.Count > 0 Then
                        Dim oRow As DataAccess.PcRichiedentiDataSet.PcRichiedentiSelectRow = oDt(0)
                        If String.IsNullOrEmpty(sId) Then
                            sRet = String.Format("Inserimento nuova stampante per il pc richiedente {0}", oRow.Nome)
                        Else
                            sRet = String.Format("Modifica stampante per il pc richiedente {0}", oRow.Nome)
                        End If
                    End If
                End Using
            End Using
        Catch ex As Exception
            _errorMessageArea.Text = "Errore la lettura dei dati del Pc Richiedente."
...
#Region "Altre DataSource"

    Private Sub TipiStampanteDataSource_Selected(sender As Object, e As ObjectDataSourceStatusEventArgs) Handles TipiStampanteDataSource.Selected
        Try
            If Not e.Exception Is Nothing Then
                My.Log.WriteException(e.Exception, TraceEventType.Error, UtilHelper.GetInnerException(e.Exception.InnerException, _className))
                _errorMessageArea.Visible = True
            ...
            Else
                ...
                '
                '
                Dim odt As DataAccess.ComboDataset.TipiStampanteComboDataTable = CType(e.ReturnValue, DataAccess.ComboDataset.TipiStampanteComboDataTable)
                If (odt Is Nothing) OrElse (odt.Rows.Count = 0) Then
                    Throw New Exception("La query non ha restituito record.")
                End If

            End If
        Catch ex As Exception
            _errorMessageArea.Text = "Errore durante la lettura dei Tipi di Stampante."
            My.Log.WriteException(ex, TraceEventType.Error, "Errore durante TipiStampanteDataSource_Selected().")
            Utility.GestisciErroriApplicationInsights(ex, "TipiStampanteDataSource_Selected")

        End Try
    End Sub

    Private Sub ServerVirtualeDataSource_Selected(sender As Object, e As ObjectDataSourceStatusEventArgs) Handles ServerVirtualeDataSource.Selected
        Try
            If Not e.Exception Is Nothing Then
                My.Log.WriteException(e.Exception, TraceEventType.Error, UtilHelper.GetInnerException(e.Exception.InnerException, _className))
                _errorMessageArea.Visible = True
            ...
            Else
                ...
                '
                '
                Dim odt As DataAccess.ComboDataset.ServerVirtualiComboDataTable = CType(e.ReturnValue, DataAccess.ComboDataset.ServerVirtualiComboDataTable)
                If (odt Is Nothing) OrElse (odt.Rows.Count = 0) Then
                    Throw New Exception("La query non ha restituito record.")
                End If
            End If
        Catch ex As Exception
            _errorMessageArea.Text = "Errore durante la lettura dei Server Virtuali."
            My.Log.WriteException(ex, TraceEventType.Error, "Errore durante ServerVirtualeDataSource_Selected().")
            Utility.GestisciErroriApplicationInsights(ex, "ServerVirtualeDataSource_Selected")
