...
-- Description:	Spostamento delle liste di attesa aperte - Le mettiamo nello store corrente
-- =============================================
CREATE PROCEDURE [dbo].[MntSpostamentoListeAttesa]
(
	@MaxNum INT = NULL
...
BEGIN
	...
	SELECT
		...
		, NumeroNosologico
	FROM 
		dbo.PartizionamentoOttieniListeAttesaFuoriStore(@MaxNum)	
	WHERE
	...
	BEGIN
		...
		BEGIN
			...
			BEGIN TRY
				...
				--
				-- Uso GETDATE() come DataPartizione in modo da spostare la lista di attesa nello store corrente
				--
				SET @DataPartizione_New = GETDATE()
	...
	PRINT 'Numero PRENOTAZIONI APERTE totali elaborate=' + CAST(@Counter AS VARCHAR(10)) + ' Success=' + CAST(@CounterSuccess AS VARCHAR(10)) + ' Error=' + CAST(@CounterError AS VARCHAR(10)) 
	PRINT 'Fine'	
END