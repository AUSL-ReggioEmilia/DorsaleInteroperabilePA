...
/* Modifico una allegato di un referto
MODIFICATO SANDRO 2015-08-19: Usa VIEW store
								Resetta i campi compressione
...
*/
CREATE PROCEDURE [dbo].[ExtAllegatiModifica](
...
Il campo @XmlAttributi e @XmlAttributi2 deve avere la seguente struttura:
    <?xml version="1.0" encoding="iso-8859-1" ?>
    <Root>
	<Attributo Nome="Dato1" Valore="3456677"/>
	<Attributo Nome="Dato1" Valore="0034577"/>
    </Root>
...
DECLARE @IdAllegatoBase uniqueidentifier
	...
	SET @IdAllegatoBase=dbo.GetAllegatiId(@IdEsternoReferto, @IdEsterno)
	IF @IdAllegatoBase IS NULL
	BEGIN
		------------------------------------------------------
		--Errore se no referto
		------------------------------------------------------
		SELECT INSERTED_COUNT = NULL
		RAISERROR('Errore allegato non trovato!', 16, 1)
		RETURN 1001
	END
		
	---
	--- Cerca il ContentType dal nume del file
	---
	...
	DECLARE @MimeDataCompresso AS VARBINARY(MAX)
...
------------------------------------------------------	
	...
	UPDATE store.AllegatiBase
	SET DataModifica=GetDate(),
		DataFile=@DataFile,
		MimeType=RTRIM(@MimeType),
		MimeData=NULL,
		MimeDataOriginale=@MimeDataOriginale,
		MimeStatoCompressione=@MimeStatoCompressione,
		MimeDataCompresso=@MimeDataCompresso
	WHERE  Id=@IdAllegatoBase
...
------------------------------------------------------	
	...
	WHERE IdAllegatiBase = @IdAllegatoBase
		AND DataPartizione = @DataPartizione
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[ExtAllegatiModifica] TO [ExecuteExt]
    AS [dbo];
