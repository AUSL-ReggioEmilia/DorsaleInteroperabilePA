...
-- =============================================
CREATE PROCEDURE [dbo].[BtCodaEventiOutputConta]
AS
BEGIN
	SET NOCOUNT ON
	...
	BEGIN TRY
		...
		IF EXISTS (SELECT * FROM [dbo].[CodaEventiOutputSemaforo] WITH(TABLOCKX) WHERE [HostBiztalk] = @HostName)
			UPDATE [dbo].[CodaEventiOutputSemaforo] WITH(TABLOCKX)
		...
		ELSE
			INSERT INTO [dbo].[CodaEventiOutputSemaforo] WITH(TABLOCKX) ([HostBiztalk], [DataLettura], [Primario])
	...
	BEGIN CATCH
		...
		SELECT @report = N'BtCodaEventiOutputConta. In catch: ' + @msg + N' xact_state:' + cast(@xact_state AS NVARCHAR(5));
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[BtCodaEventiOutputConta] TO [ExecuteBiztalk]
    AS [dbo];
