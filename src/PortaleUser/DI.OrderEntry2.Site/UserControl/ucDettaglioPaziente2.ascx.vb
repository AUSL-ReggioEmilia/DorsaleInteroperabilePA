...
Public Class ucDettaglioPaziente2
...
#End Region
	...
	Protected Function GetNosologico() As String
		Dim nosologico As String = String.Empty

		If Not String.IsNullOrEmpty(Me.Nosologico) Then
			nosologico = $"<b>Episodio:</b>&nbsp {Me.Nosologico}"
		End If

		Return nosologico
	End Function

	Public Function GetElement(oEpisodio As Object, oConsensoAziendale As Object) As String
		'
		...
		Try
			If Not String.IsNullOrEmpty(Me.Nosologico) Then
				...
				If oEpisodio IsNot Nothing Then
					Dim episodio As WcfDwhClinico.EpisodioType = CType(oEpisodio, WcfDwhClinico.EpisodioType)

					If episodio IsNot Nothing Then
						If episodio.AziendaErogante IsNot Nothing Then
							Dim aziendaerogante As String = episodio.AziendaErogante
							Dim consensoAziendale As WcfDwhClinico.PazienteConsensoType = CType(oConsensoAziendale, WcfDwhClinico.PazienteConsensoType)
							resultElement = GetSimboloTipoEpisodioRicovero(episodio, consensoAziendale)
	...
	Private Function GetSimboloTipoEpisodioRicovero(Episodio As WcfDwhClinico.EpisodioType, ConsensoAziendale As WcfDwhClinico.PazienteConsensoType) As String
		'
		' Ottiene l'Image dell'episodio da mostrare nella testata del paziente
		'
		Dim strHtml As String = String.Empty
		If Not Episodio.TipoEpisodio Is Nothing AndAlso String.Compare(Episodio.Categoria, "Ricovero", True) = 0 Then
			Dim sTipoEpisodioRicovero As String = CType(Episodio.TipoEpisodio.Codice, String).ToUpper
			If Not String.IsNullOrEmpty(sTipoEpisodioRicovero) AndAlso Not ConsensoAziendale Is Nothing AndAlso Not String.IsNullOrEmpty(ConsensoAziendale.Codice) Then
				Dim eConsenso As ConsensoMinimoAccordato = CType(ConsensoAziendale.Codice, ConsensoMinimoAccordato)
				'
				...
				If eConsenso = ConsensoMinimoAccordato.DossierStorico Then
					...
					If Not Episodio.DataConclusione.HasValue AndAlso Episodio.StrutturaConclusione Is Nothing Then
						'
						' Solo se il ricovero è ancora in corso e quindi solo se la DataConclusione e la StrutturaConclusione sono nothing
						'
						If Episodio.DataApertura.HasValue AndAlso Not Episodio.StrutturaUltimoEvento Is Nothing Then
							If Episodio.DataUltimoEvento.HasValue Then
								sTooltip = String.Format("Ricoverato nel reparto {0} dal {1:d}", Episodio.StrutturaUltimoEvento.Descrizione, Episodio.DataUltimoEvento.Value)
							Else
								sTooltip = String.Format("Ricoverato nel reparto {0}", Episodio.StrutturaUltimoEvento.Descrizione)
							End If
						End If
					Else
						'
						...
						If Episodio.DataConclusione.HasValue Then
							sTooltip = String.Format("Dimesso dal reparto {0} il {1:d}", Episodio.StrutturaConclusione.Descrizione, Episodio.DataConclusione.Value)
						Else
							sTooltip = String.Format("Dimesso dal reparto {0}", Episodio.StrutturaConclusione.Descrizione)
						End If
					End If
		...
		Try
			If Not idPaziente Is Nothing Then
                ...
                If hasEsenzioni Then
                    Dim linkbutton As LinkButton = CType(fvPaziente.FindControl("lnkEsenzioni"), LinkButton)
...
#Region "Referti"
    ...
    Public Sub SetLinkRefertiVisibility(idPaziente As String)
        Try
            ...
            Dim imgReferto As HtmlControls.HtmlImage = CType(fvPaziente.FindControl("imgReferti"), HtmlControls.HtmlImage)
            Dim linkButtonReferti As LinkButton = CType(fvPaziente.FindControl("lnkReferti"), LinkButton)

            'Verifico che siano stati trovati i controlli
            If imgReferto IsNot Nothing AndAlso linkButtonReferti IsNot Nothing Then

                'Ottengo il periodo dell'ultimo referto
                Dim ds As New CustomDataSourceDettaglioPaziente
                periodoReferto = ds.GetPeriodoUltimoReferto(idPaziente)

                'Valorizzo l'url dell'immagine
                Select Case periodoReferto
                    Case TipoReferti.GiornoCorrente
                        linkButtonReferti.Visible = True
                        imgReferto.Attributes.Item("src") = VirtualPathUtility.ToAbsolute("~/Images/PresenzaReferti1.gif")
                    Case TipoReferti.UltimaSettimana
                        linkButtonReferti.Visible = True
                        imgReferto.Attributes.Item("src") = VirtualPathUtility.ToAbsolute("~/Images/PresenzaReferti7.gif")
                    Case TipoReferti.UltimoMese
                        linkButtonReferti.Visible = True
                        imgReferto.Attributes.Item("src") = VirtualPathUtility.ToAbsolute("~/Images/PresenzaReferti30.gif")
                End Select
            End If
...
#Region "Ricoveri"
	...
	'Private Sub odsPazienteDettaglio_Selected(sender As Object, e As ObjectDataSourceStatusEventArgs) Handles odsPaziente.Selected
	'	Try
...
#End Region
	...
	Private Sub fvPaziente_PreRender(sender As Object, e As EventArgs) Handles fvPaziente.PreRender
		Try
			SetLinkEsenzioniVisibility(Me.IdPaziente)
            SetLinkRefertiVisibility(Me.IdPaziente)
        Catch ex As Exception
