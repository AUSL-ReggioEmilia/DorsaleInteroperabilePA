...
namespace OrderEntryPlanner.Pages
{
    public partial class DettaglioOrdine : System.Web.UI.Page
    {
        ...
        public Guid? IdSac
        {
            ...
            set
            {
                //Valorizzo il viewState inserendo il parametro IdSac
        ...
        public string GetInfoPaziente(object paziente2)
        {
            ...
            try
            {
                ...
                {
                    result += string.Format(". Deceduto il <label class='text-danger'>{0:d}</label>.", paziente.Generalita.DataDecesso);
                }
            }
            catch (ApplicationException ex)
            {
        ...
        public string GetPazienteSacUrl()
        {
            ...
            try
            {
                ...
                result = Properties.Settings.Default.SacUrl + "/Pazienti/PazienteDettaglio.aspx?id=" + IdSac;
        ...
        protected void btnRipianifica_Click(object sender, EventArgs e)
        {
            try
            {
                //Eseguo il redirect alla pagina di pianificazione passando in querystring l'IdOrdine e il Paret (la pagina di origine)
                Response.Redirect($"PianificaOrdine?IdOrdine={IdOrdine}&Parent=Detail", false);
            }
            catch (ApplicationException ex)
            {
                Master.showAlert(ex.Message);
        ...
        protected void odsPaziente_Selected(object sender, ObjectDataSourceStatusEventArgs e)
        {
            try
            {
                ...
                PazienteType paziente = (PazienteType)e.ReturnValue as PazienteType;
        ...
        protected void BtnPianifica_Click(object sender, EventArgs e)
        {
            try
            {
